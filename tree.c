...
FileNode* buildTree(const char *path) {
    ...
    base = (base && base[1] != '\0') ? base + 1 : path;
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        // 7. 将得到的子结点存入临时数组
        if (child) {
            FileNode **newChildren = (FileNode**)realloc(children, (childCount + 1) * sizeof(FileNode*));
            if (!newChildren) {
...
char* getBaseName(void) {
    ...
    char *res = (base && base[1] != '\0') ? strdup(base + 1) : strdup(cwd);
    free(cwd);
    return res;
}
